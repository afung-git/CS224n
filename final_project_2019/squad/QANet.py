...
# code from starter code
from util import masked_softmax
from layers import MultiHeadSelfAttention, make_mask
...
class EncoderBlock(nn.Module):
    def __init__(self, conv_num, d_model, num_head, k, dropout=0.1):
        ...
        self.self_att = MultiHeadSelfAttention(num_head, d_model, drop_prob=dropout)
    ...
    def forward(self, x, mask, l, blks):
        ...
        for i, conv in enumerate(self.convs):
            ...
            if (i) % 2 == 0:
                out = F.dropout(out, p=dropout, training=self.training)  # Alternate
            out = conv(out)
        ...
        # print(out.shape, mask.shape)  # B x H x L, B x L
        mask = make_mask(mask)  # B x L x L
        out = self.self_att(out.transpose(1,2), out.transpose(1,2), out.transpose(1,2), mask).transpose(1,2)
        out = self.layer_dropout(out, res, dropout*float(l)/total_layers)
        l += 1
        res = out
...
class QANet(nn.Module):
    def __init__(self, word_mat, char_mat,
        ...
        self.model_enc_blks = nn.ModuleList([EncoderBlock(conv_num=2, d_model=d_model, num_head=num_head, k=7, dropout=0.1) for _ in range(3)])
