...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        gap = max(0, max_length - len(sentence))
        mask = [True]*(max_length - gap) + [False]*gap
        sentence = sentence[:max_length] + [zero_vector]*gap
        labels = labels[:max_length] + [zero_label]*gap
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        self.input_placeholder = tf.placeholder(tf.int32, (None, self.max_length, Config.n_features), "input_placeholder")
        self.labels_placeholder = tf.placeholder(tf.int32, (None, self.max_length), "labels_placeholder")
        self.mask_placeholder = tf.placeholder(tf.bool, (None, self.max_length), "mask_placeholder")
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        embeddings = tf.nn.embedding_lookup(tf.Variable(initial_value=self.pretrained_embeddings), self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        b2 = tf.get_variable( name="b2", shape=[Config.n_classes], initializer = tf.zeros_initializer() )
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o = tf.nn.dropout(o, dropout_rate)
                prediction = tf.matmul(o,U) + b2
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        loss = tf.reduce_mean( tf.boolean_mask( tf.nn.sparse_softmax_cross_entropy_with_logits(labels = self.labels_placeholder, logits = preds), self.mask_placeholder) )
        ### END YOUR CODE
        return loss
