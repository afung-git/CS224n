...
def make_windowed_data(data, start, end, window_size = 1):
	...
	### YOUR CODE HERE (5-20 lines)
        ...
        for i in range(len(sentence)):
            features = [feat for word in padded_sentence[i:i+2*window_size+1] for feat in word]
            windowed_data.append((features, labels[i]))    
	### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(tf.int32, (None, self.config.n_window_features), name="inputs")
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~3-5 lines)
        embed_tensor = tf.Variable(initial_value = self.pretrained_embeddings)
        lookup = tf.nn.embedding_lookup(embed_tensor, self.input_placeholder)
        embeddings = tf.reshape(lookup, [-1, self.config.n_window_features*self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable( name='W', shape=[self.config.n_window_features*self.config.embed_size, self.config.hidden_size], initializer=tf.contrib.layers.xavier_initializer() )
        b1 = tf.get_variable( name='b1', shape=[self.config.hidden_size], initializer=tf.zeros_initializer() )
        U = tf.get_variable( name='U', shape=[self.config.hidden_size, self.config.n_classes], initializer=tf.contrib.layers.xavier_initializer() )
        b2 = tf.get_variable( name='b2', shape=[self.config.n_classes], initializer=tf.zeros_initializer() )

        h = tf.nn.relu(tf.matmul(x,W) + b1)
        h_drop = tf.nn.dropout(h, dropout_rate) # don't 1- dropout_rate
